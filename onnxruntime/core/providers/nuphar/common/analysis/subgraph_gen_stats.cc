...
int SubGraphStats::NodeUseCount(const onnxruntime::Node* node) const {
  ...
  return UseCountAnalysis::Promote(passes_[UseCountAnalysisOffset])->NodeUseCount(node);
}
  ...
  if (IsFusedNode(node)) {
    ...
    for (auto def : node.OutputDefs()) {
      if (std::find(graph_outputs.begin(), graph_outputs.end(), def) == graph_outputs.end()) {
        UseCountAnalysis::Promote(passes_[UseCountAnalysisOffset])->IncrementCount(def);
      }
    }

  } else {
    ORT_ENFORCE(passes_.size() > OutputAliasAnalysisOffset);
    OutputAliasAnalysis::Promote(passes_[OutputAliasAnalysisOffset])->EvaluateSingleNode(node);
  }
}

bool SubGraphStats::IsOutputNode(const onnxruntime::Node* node) const {
  ORT_ENFORCE(passes_.size() > OutputAliasAnalysisOffset);
  return OutputAliasAnalysis::Promote(passes_[OutputAliasAnalysisOffset])->IsOutputNode(node);
}

bool SubGraphStats::IsOutputAlias(const onnxruntime::Node* node) const {
  ORT_ENFORCE(passes_.size() > OutputAliasAnalysisOffset);
  return OutputAliasAnalysis::Promote(passes_[OutputAliasAnalysisOffset])->IsOutputAlias(node);
}

const onnxruntime::NodeArg* SubGraphStats::SourceDefOfOutputAlias(const onnxruntime::NodeArg* node) const {
  ORT_ENFORCE(passes_.size() > OutputAliasAnalysisOffset);
  return OutputAliasAnalysis::Promote(passes_[OutputAliasAnalysisOffset])->SourceDefOfOutputAlias(node);
}
