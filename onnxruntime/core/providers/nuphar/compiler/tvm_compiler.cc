...
Status TVMCompiler::BuildSubgraph() {
  ...
  for (const auto& def : subgraph->GetOutputs()) {
    auto input_def = codegen::SubGraphStats::Promote(context_.GetGraphStats())->SourceDefOfOutputAlias(def);
    if (input_def) {
...
// a temp fuction to create PackedFuncInfo
// And do outPut alias
// TODO: move this out of TVMCompiler.
void TVMCompiler::FillPackedFuncInfo(PackedFuncInfo* ctx_func) {
  size_t input_count =
      node_.InputDefs().size() +
      context_.SizeInitializerMarshalled() -
      num_initializers_in_graph_inputs_;

  size_t output_count = node_.OutputDefs().size();
  auto& output_tvm_arg_idx = ctx_func->output_tvm_arg_idx;
  auto& output_def_alias = ctx_func->output_def_alias;

  // Construct output_alias_ lookup table
  size_t num_actual_outputs = 0;
  std::map<NodeKey, int> visited_state_alias_def;
  for (size_t i_def = 0; i_def < output_count; ++i_def) {
    auto def = node_.OutputDefs()[i_def];
    auto input_def = codegen::SubGraphStats::Promote(context_.GetGraphStats())->SourceDefOfOutputAlias(def);
    if (input_def) {
      auto key = GetKey(input_def);
      auto iter = visited_state_alias_def.find(key);
      if (iter == visited_state_alias_def.end()) {
        visited_state_alias_def.insert(std::make_pair(key, i_def));
      } else {
        output_tvm_arg_idx.push_back(OutputAliased);
        output_def_alias.push_back(iter->second);
        continue;
      }
    }
    output_def_alias.push_back(i_def);
    output_tvm_arg_idx.push_back(input_count + num_actual_outputs);
    ++num_actual_outputs;
  }

  ctx_func->input_count = input_count;
  ctx_func->output_count = output_count;
  ctx_func->num_actual_outputs = num_actual_outputs;
  ctx_func->type_codes.resize(ctx_func->num_args, kNDArrayContainer);

  ctx_func->tvm_outputs = tvm_outputs_;
}

// Lower compiles the tvm::Tensor to a function
Status TVMCompiler::Lower(tvm::Target tvm_target,
                          tvm::Target tvm_host_target,
                          PackedFuncInfo* ctx_func) {
  // ctx_func cannot be nullptr
  ORT_RETURN_IF_NOT(nullptr != ctx_func);
  ...
  int num_args = gsl::narrow_cast<int>(tvm_args_.size());

  ctx_func->device_type = static_cast<DLDeviceType>(tvm_target->device_type);
  ctx_func->packed_func = cached_func;
  ctx_func->num_args = num_args;
  ctx_func->name = func_name;
  ctx_func->type_codes.resize(num_args, kNDArrayContainer);

  FillPackedFuncInfo(ctx_func);
