...
Status NUPHAR_TVM_X86_OP_IR_CREATOR_CLASS(MatMulInteger)::Evaluate(
  ...
  NupharCodeGenCtx* ctx_nuphar = NupharCodeGenCtx::Promote(&ctx_codegen);
  ...
  if (rhs_tensor->shape.size() == 2) {
    ...
    if (p_input_dim && p_embed_dim) {
      ...
      if (is16bitSymm || is8bitAsymm) {
        ...
        bool support_AVX2 = CPUIDInfo::GetCPUIDInfo().HasAVX2();
        auto output_tensors =
            is16bitSymm ? support_AVX2 ? nuphar_codegen::QMatMulSymmetricAVX2(quantized_marshalled, lhs_tensor,
                        ...
                        : support_AVX2 ? nuphar_codegen::QMatMulAsymmetricAVX2(quantized_marshalled, lhs_tensor,
                                                                               batch_seq_dim, input_dim, embed_dim,
