...
NupharCodeGenCtx::NupharCodeGenCtx(
  ...
  if (nullptr != subgraph && !IsFusedNode(node)) {
    codegen::SubGraphStats::Promote(graph_stats_)->Evaluate(GraphViewer(*subgraph));
  } else {
    codegen::SubGraphStats::Promote(graph_stats_)->EvaluateSingleNode(node);
  }
}
...
void NupharCodeGenCtx::RecordTensorToNode(const tvm::Tensor& t, const Node* node) {
  ...
  if (!codegen::SubGraphStats::Promote(graph_stats_)->IsOutputAlias(node))
