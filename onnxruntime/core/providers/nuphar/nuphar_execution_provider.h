...
class NupharExecutionProvider : public IExecutionProvider {
 public:
  ...
  const tvm_codegen::NupharCodeGenHandle* GetTVMCodeGenSetting() const {
    ORT_ENFORCE(tvm_codegen_handles_.size() > 0);
    return tvm_codegen_handles_.front().get();
  }
 ...
 private:
  ...
  // tvm_codegen_handles_ holds a list of codegen env.
  // Why a list? it is for multi-target support later
  // TODO make sure > 1 work
  std::vector<std::unique_ptr<tvm_codegen::NupharCodeGenHandle>> tvm_codegen_handles_;
