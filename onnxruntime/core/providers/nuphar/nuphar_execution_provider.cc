...
NupharExecutionProvider::NupharExecutionProvider(const NupharExecutionProviderInfo& info)
  ...
  handle->enable_per_node_parallelized = info.enable_per_node_parallel;  // TODO remove
  handle->shape_inference = whole_graph_shape_infer_;
  handle->allow_unaligned_buffers = info.allow_unaligned_buffers;
  tvm_codegen_handles_.push_back(std::move(handle));
}
...
NupharExecutionProvider::GetCapability(const onnxruntime::GraphViewer& graph_viewer,
  ...
  // construct graph
  codegen::GraphPartitionStats::Promote(graph_stats_)->SetShapeInference(whole_graph_shape_infer_);
  ...
  for (size_t i = num_existing_nodes; i < compiled_nodes_.size(); ++i) {
    ...
    info.compute_func =
        [](FunctionState state, ONNXRunTimeTensor* input_tensors, size_t num_inputs,
          ...
          s->Compute(input_tensors, num_inputs, output_tensors, num_outputs);
          return 0;
        };
