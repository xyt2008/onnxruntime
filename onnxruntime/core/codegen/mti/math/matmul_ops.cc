...
tvm::Tensor MatMul(const tvm::Tensor& A, const tvm::Tensor& B, const std::string& name) {
  ...
  if (a_rank == 2 && b_rank == 2) {
    ...
    for (int64_t i = 0; i < output_rank - 2; i++) {
      if (output_rank - i > a_rank) {
        output_shape.push_back(B_shape[i]);
      } else if (output_rank - i > b_rank) {
        output_shape.push_back(A_shape[i]);
      } else {
        auto a_dim = A_shape[a_rank + i - output_rank];
        auto b_dim = B_shape[b_rank + i - output_rank];
        auto* pa = tvm::as_const_int(a_dim);
        auto* pb = tvm::as_const_int(b_dim);
        if (pa == nullptr || *pa > 1) {
          MTI_ASSERT((pa != nullptr && pb != nullptr && *pa == *pb) || a_dim.same_as(b_dim) || (pb != nullptr && *pb == 1));
          output_shape.push_back(a_dim);
        } else {
          MTI_ASSERT((pa != nullptr && pb != nullptr && *pa == *pb) || a_dim.same_as(b_dim) || (pa != nullptr && *pa == 1));
